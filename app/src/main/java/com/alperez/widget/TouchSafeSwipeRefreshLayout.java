 ...
 */
public class TouchSafeSwipeRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
     ...
     * @param context
     */
    public TouchSafeSwipeRefreshLayout(Context context) {
        this(context, null);
    }
     ...
     * @param attrs
     */
    public TouchSafeSwipeRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private String motionEventToShortText(MotionEvent ev) {
        return String.format("MotionEvent{action=%s, x=%.1f, y=%.1f}", MotionEvent.actionToString(ev.getActionMasked()), ev.getX(0), ev.getY(0));
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || mReturningToStart || mRefreshing ) {
            ...
            Log.e(LOG_TAG, "Fail because of not enabled OR refreshing OR returning to start. "+motionEventToShortText(ev));
            return false;
        }


        /*if (mNestedScrollInProgress || canChildScrollUp()) {
            Log.e(LOG_TAG, "Fail because of nested content is Scrolling. "+motionEventToShortText(ev));
            return false;
        }*/
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                //TODO mIsBeingDragged = false;

                if ((pointerIndex = ev.findPointerIndex(mActivePointerId)) >= 0) {

                    if (mNestedScrollInProgress || canChildScrollUp()) {
                        Log.e(LOG_TAG, "Fail because of nested content is Scrolling. Set pending DOWN=true. "+motionEventToShortText(ev));
            ...
            case MotionEvent.ACTION_MOVE:
                if (mActivePointerId == INVALID_POINTER) {
                    ...
                    Log.e(LOG_TAG, "Gesture was declined previously because of horizontal swipe");
                    return false;
                    ...
                    Log.e(LOG_TAG, "Consider this move as down - setup initial X/Y."+motionEventToShortText(ev));
                    ...
                    Log.e(LOG_TAG, "Decline gesture because of horizontal swipe");
                    return false;
                }
                ...
                startDragging(y);
                if (!mIsBeingDragged) {
                    Log.d(LOG_TAG, "Waiting for dY to start dragging. "+motionEventToShortText(ev));
                } else {
                    Log.d(LOG_TAG, "Dragging started! "+motionEventToShortText(ev));
                }
                break;
    ...
    public interface OnChildScrollUpCallback {
         ...
         */
        boolean canChildScrollUp(TouchSafeSwipeRefreshLayout parent, @Nullable View child);
    }
}
